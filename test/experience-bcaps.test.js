...
test('grounded requires tool-verified tech in a contributing session; "verified" never appears', () => {
  const states = [
    mkState({
      cwd: 'C:\\w\\a',
      gitRoot: 'C:\\w\\a',
      cls: cls({ bcaps: ['lead-generation'], techs: [['Python', 'hands_on', true]] }),
    }),
    mkState({
      cwd: 'C:\\w\\b',
      gitRoot: 'C:\\w\\b',
      cls: cls({ bcaps: ['lead-generation'], techs: [['Python', 'hands_on', false]] }),
    }),
  ];
  const { practitioners } = experience(states);
  ...
  for (const p of c.projects)
    assert.deepStrictEqual(offending(p), [], 'no verified-named key on the trace');
});
...
test('non-professional and unknown-category sessions earn no business capability credit', () => {
  const states = [
    mkState({
      cwd: 'C:\\w\\l',
      gitRoot: 'C:\\w\\l',
      cls: cls({ category: 'learning', bcaps: ['seo-diagnostics'] }),
    }),
    mkState({
      cwd: 'C:\\w\\p',
      gitRoot: 'C:\\w\\p',
      cls: cls({ category: 'personal', bcaps: ['seo-diagnostics'] }),
    }),
    ...
    mkState({
      cwd: 'C:\\w\\u',
      gitRoot: 'C:\\w\\u',
      cls: cls({ category: 'unknown', via: 'heuristic', bcaps: ['seo-diagnostics'] }),
    }),
  ];
  const { practitioners } = experience(states);
...
test('singleton catch-alls collapse to ONE misc business-capability credit', () => {
  ...
  for (let i = 0; i < 4; i++) {
    states.push(
      mkState({ cwd: 'C:\\Users\\sampleuser', cls: cls({ bcaps: ['cloud-operations'] }) })
    );
  }
  const { practitioners } = experience(states);
...
test('inherited provenance and min confidence ride raw', () => {
  const states = [
    mkState({
      cwd: 'C:\\w\\i',
      gitRoot: 'C:\\w\\i',
      cls: cls({ via: 'inherited', confidence: 0.4, bcaps: ['billing-systems'] }),
    }),
    mkState({
      cwd: 'C:\\w\\i',
      gitRoot: 'C:\\w\\i',
      cls: cls({ confidence: 0.9, bcaps: ['billing-systems'] }),
    }),
  ];
  const { practitioners } = experience(states);
  ...
  const states = [
    mkState({
      ...
      cls: cls({
        ...
        bcaps: ['lead-generation'],
        techs: [['Python', 'hands_on', true]],
      }),
    }),
  ];
  const { practitioners } = experience(states);
...
test('capability_coverage counts capability-eligible professional sessions; heuristic and non-professional excluded', () => {
  const states = [
    ...
    mkState({
      cwd: 'C:\\p\\a',
      gitRoot: 'C:\\p\\a',
      cls: cls({ category: 'learning', bcaps: [] }),
    }), // not professional
    mkState({
      cwd: 'C:\\p\\a',
      gitRoot: 'C:\\p\\a',
      cls: cls({ via: 'inherited', bcaps: ['process-automation'] }),
    }), // inherited counts
  ];
  const { practitioners } = experience(states);
