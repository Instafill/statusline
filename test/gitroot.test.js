...
const git = (cwd, ...args) =>
  spawnSync('git', ['-C', cwd, ...args], { encoding: 'utf8', windowsHide: true });
const haveGit =
  spawnSync('git', ['--version'], { encoding: 'utf8', windowsHide: true }).status === 0;

test(
  'main worktree reports a root and no worktree name; a linked worktree reports both',
  { skip: !haveGit },
  () => {
    ...
    assert.equal(
      ...
      'origin captured, credentials stripped'
    );
    ...
    assert.equal(
      (linked.main_root || '').toLowerCase(),
      main.root.toLowerCase(),
      'main_root points at the parent repo'
    );
    assert.equal(
      linked.origin,
      'https://github.com/acme/demo.git',
      'worktree shares the repo origin'
    );
