...
async function api(p) {
  ...
  try {
    ...
    console.error(`Start it with: node <repo>/src/cli.js start   (lock file: ${path.join(HOME, 'watcher.lock')})`);
    process.exit(3);
  }
  if (!res.ok) {
...
const pad = (v, n) => String(v === null || v === undefined ? '-' : v).slice(0, n).padEnd(n);
...
const commands = {
  async status() {
    ...
    console.log(`agent        ${recordedAgentRoot() || repoFrom(s.installer.hookCommand) || '(unknown)'}`);
    console.log(`sessions     ${s.sessions}`);
    console.log(`spool        ${s.spool.pending} pending, ${s.spool.quarantined} quarantined`);
    console.log(`hooks        ${s.installer.entries.length - missing.length}/${s.installer.entries.length} installed` +
      (missing.length ? ` — MISSING: ${missing.map((e) => e.event).join(', ')} (run: node <repo>/src/cli.js install)` : ''));
    const q = s.scheduler || {};
    console.log(`scheduler    queued=${q.queued ?? '-'} running=${q.running ?? '-'} classified=${q.classified ?? '-'} failed=${q.failed ?? '-'}` +
  ...
  async sessions(daysArg) {
    ...
    console.log(`${pad('LAST', 10)} ${pad('STATE', 14)} ${pad('PROJECT', 18)} ${pad('TURNS', 5)} WORK`);
    for (const s of rows) {
      const c = s.classification;
      const note = c ? c.work_type : s.outlook && s.outlook.kind ? `(${s.outlook.kind})` : '';
      const flag = s.label ? ` [${s.label}]` : '';
      console.log(`${pad(day(s.last_event_at), 10)} ${pad(s.classification_state, 14)} ${pad(s.project && s.project.name, 18)} ${pad(s.counts.turns, 5)} ${note}${flag}`);
    }
  ...
  async session(sid) {
    ...
    console.log(`git          ${s.git_root || '-'}${s.git_worktree ? ` (worktree ${s.git_worktree})` : ''}`);
    ...
    console.log(`window       ${s.created_at} → ${s.last_event_at}${s.live ? '  (host process still running)' : ''}`);
    console.log(`counts       ${JSON.stringify(s.counts)}`);
    console.log(`state        ${s.classification_state}${s.classifier_error ? ` — ${s.classifier_error}` : ''}   idle_minutes=${s.idle_minutes}`);
    if (Object.keys(c).length) {
      console.log(`category     ${c.work_category}  depth=${c.work_depth}  confidence=${c.confidence}`);
      console.log(`work_type    ${c.work_type || '-'}`);
      console.log(`technologies ${(c.technologies || []).map((t) => (t.name || t) + (t.evidence ? `(${t.evidence}${t.verified ? '' : '?'})` : '')).join(', ') || '-'}`);
    }
    const meta = s.classification && s.classification._meta;
    if (meta) console.log(`classifier   ${meta.classifier || '-'}  ${meta.model || ''} ${meta.cost_usd ? `$${meta.cost_usd}` : ''}`);
  ...
  async projects() {
    ...
    console.log(`${real.length} project(s)${singles ? ` (+${singles} miscellaneous singleton session(s), not counted as projects)` : ''}\n`);
    console.log(`${pad('ID', 16)} ${pad('NAME', 24)} ${pad('SESS', 5)} ${pad('KIND', 10)} ${pad('ENGAGEMENT', 14)} LAST`);
    for (const p of real) {
      const a = p.aggregate || {};
      console.log(`${pad(p.id, 16)} ${pad(p.name, 24)} ${pad(p.session_ids ? p.session_ids.length : '-', 5)} ${pad(p.key && p.key.kind, 10)} ${pad(p.engagement_id, 14)} ${day(a.last_seen)}`);
    }
    const merges = g.projects.flatMap((p) => (p.suggested_merges || []).map((m) => ({ from: p.id, ...m })));
    if (merges.length) {
      console.log(`\n${merges.length} merge suggestion(s):`);
      for (const m of merges) console.log(`  ${m.from} + ${m.project_id || m.other || m.id}  score=${m.score}  ${(m.reasons || []).join(', ')}`);
    }
  },

  async egress(n) {
    const rows = (await api('/api/egress')).slice().sort((a, b) => String(a.at).localeCompare(String(b.at)));
    const take = Number(n) > 0 ? Number(n) : 20;
    const failed = rows.filter((r) => r.outcome && r.outcome !== 'ok');
    console.log(`${rows.length} recent attempt(s), ${failed.length} not ok; newest ${rows.length ? rows[rows.length - 1].at : '-'}\n`);
    console.log(`${pad('AT', 24)} ${pad('KIND', 10)} ${pad('OUTCOME', 10)} ${pad('HTTP', 5)} DETAIL`);
    for (const r of rows.slice(-take)) {
      const detail = [r.endpoint_host, r.session_count !== undefined ? `${r.session_count} session(s)` : null, r.error || r.reason]
        .filter(Boolean)
        .join('  ');
      console.log(`${pad(r.at, 24)} ${pad(r.kind, 10)} ${pad(r.outcome, 10)} ${pad(r.http_status, 5)} ${detail}`);
    }
  },
  ...
  async repo() {
    ...
    if (!r) {
      console.error('could not locate the agent — nothing recorded in agent.json and no hook command to read it from');
      process.exit(5);
    }
    console.log(r);
  },
...
function usage(extra) {
  ...
  console.error('  sessions [days]     one line per session active in the last N days (default 14)');
